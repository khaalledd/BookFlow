...
export class BookingCreatedListener {
  ...
  handleBookingCreated(payload: {
    ...
    this.logger.log(
      `[BookFlow] Booking confirmed — ${payload.serviceName} at ${payload.businessName} on ${payload.date} ${payload.startTime}`,
    );
    if (payload.customerEmail) {
      this.logger.log(
        `[BookFlow] Confirmation SMS/email queued for customer: ${payload.customerEmail}`,
      );
    }
